...
# Description: While baseline generation is simply done with the CubicSplines function 
# from scipy, this script defines the function which allows the user to click on a plot 
# of the raw data in order to select anchor points for a particular material's spectra.
# The user can then save these anchor points in the "anchor_points" section of the 
# materials.json file for future use.
# The Baseline Parameters section of dataframe.csv will show the relevant anchor points 
# for each material, which will be used to generate the baseline.
...
def _cleanup_widgets():
    """
    Cleans up any lingering ipywidgets or Plotly FigureWidget objects from previous 
    runs.
    This prevents state errors and memory leaks when running interactive selection 
    multiple times in Jupyter/VS Code.
    """
    try:
        import gc
...
def select_anchor_points(
    ...
    def show_baseline_preview():
        """
        Helper function:
        Shows a preview of the cubic spline baseline and baseline-corrected spectrum 
    ...
    def reset_selection():
        """
        Helper function:
        Clears anchor points and resets the interactive plot and widgets for a new 
        selection.
        """
    ...
    def continue_callback(b):
        """
        Helper function:
        Handles the Continue button for the initial anchor selection.
        If enough points are selected, shows the baseline preview; otherwise, prompts 
        the user to select more points.
        """
        button_box_out.clear_output()
        with done:
            clear_output()
